...
Func _Console_AddAlias($sSource, $sTarget, $sExeName = Default, $fUnicode = Default, $hDll = -1)
	...
	Local $aResult = DllCall($hDll, "bool", "AddConsoleAliasW", _
			"wstr", $sSource, _
			"wstr", $sTarget, _
			"wstr", $sExeName)
...
Func _Console_FillOutputCharacter($hConsoleOutput, $sCharacter, $nLength, $iX = 0, $iY = 0, $fUnicode = Default, $hDll = -1)
	...
	Local $aResult = DllCall("kernel32.dll", "bool", "FillConsoleOutputCharacterW", _
			"handle", $hConsoleOutput, _
			"byte", AscW($sCharacter), _
			"dword", $nLength, _
			"int", BitShift($iY, -16) + $iX, _
			"dword*", 0)
	If @error Then Return SetError(@error, @extended, False)
...
Func _Console_GetAlias($sSource, $sExeName = -1, $fUnicode = Default, $hDll = -1)
	...
	Local $tTargetBuffer = DllStructCreate("wchar buffer[1024]")

	Local $aResult = DllCall($hDll, "dword", "GetConsoleAliasW", _
			"wstr", $sSource, _
...
Func _Console_GetAliases($sExeName = -1, $fUnicode = Default, $hDll = -1)
	...
	Local $tAliasBuffer = DllStructCreate("wchar buffer[" & _Console_GetAliasesLength($sExeName, $fUnicode, $hDll) & "]")

	Local $aResult = DllCall($hDll, "dword", "GetConsoleAliasesW", _
...
Func _Console_GetAliasesLength($sExeName = -1, $fUnicode = Default, $hDll = -1)
	...
	Local $aResult = DllCall($hDll, "dword", "GetConsoleAliasesLengthW", _
			"wstr", $sExeName)
	If @error Then Return SetError(@error, @extended, 0)
...
Func _Console_GetAliasExes($fUnicode = Default, $hDll = -1)
	...
	Local $tExeNameBuffer = DllStructCreate("wchar buffer[" & _Console_GetAliasExesLength(True, $hDll) & "]")

	Local $aResult = DllCall($hDll, "dword", "GetConsoleAliasExesW", _
...
Func _Console_GetAliasExesLength($fUnicode = Default, $hDll = -1)
	...
	Local $aResult = DllCall($hDll, "dword", "GetConsoleAliasExesLengthW")
	If @error Then Return SetError(@error, @extended, 0)
...
Func _Console_GetOriginalTitle($fUnicode = Default, $hDll = -1)
	...
	Local $tConsoleTitle = DllStructCreate("wchar buffer[128]")

	Local $aResult = DllCall($hDll, "dword", "GetConsoleOriginalTitleW", _
...
Func _Console_GetScreenBufferPos($hConsoleOutput = -1, $hDll = -1)
	Local $tConsoleScreenBufferInfo = _Console_GetScreenBufferInfo($hConsoleOutput, $hDll)
	If @error Then Return SetError(@error, @extended, 0)
	Local $aRet[4] = [$tConsoleScreenBufferInfo.Left, $tConsoleScreenBufferInfo.Top, $tConsoleScreenBufferInfo.Right - $tConsoleScreenBufferInfo.Left, $tConsoleScreenBufferInfo.Bottom - $tConsoleScreenBufferInfo.Top]
	Return $aRet
...
Func _Console_GetSelectionRect($hDll = -1)
	...
	Local $aRet[4] = [$tConsoleSelectionInfo.Left, $tConsoleSelectionInfo.Top, $tConsoleSelectionInfo.Right - $tConsoleSelectionInfo.Left, $tConsoleSelectionInfo.Bottom - $tConsoleSelectionInfo.Top]
	Return $aRet
...
Func _Console_GetTitle($fUnicode = Default, $hDll = -1)
	...
	Local $aResult = DllCall($hDll, "dword", "GetConsoleTitleW", _
			"ptr", 0, _
			"dword", 0)
	If @error Then Return SetError(@error, @extended, "")

	Local $tConsoleTitle = DllStructCreate("wchar buffer[" & $aResult[0] & "]")
...
Func _Console_ReadConsoleEx($hConsoleInput, $nNumberOfCharsToRead, $fUnicode = Default, $hDll = -1)
	...
	Local $tBuffer = DllStructCreate("wchar buffer[" & $nNumberOfCharsToRead & "]")

	Local $aResult = DllCall($hDll, "BOOL", "ReadConsoleW", _
			"handle", $hConsoleInput, _
			"struct*", $tBuffer, _
...
Func _Console_ReadConsoleInput($hConsoleInput, $pBuffer, $iLength, $fUnicode = Default, $hDll = -1)
	...
	Local $aResult = DllCall($hDll, "BOOL", "ReadConsoleInputW", _
			"handle", $hConsoleInput, _
			"ptr", $pBuffer, _
			"int", $iLength, _
			"int*", 0)
...
Func _Console_ReadConsoleInputRecord($hConsoleInput = -1, $fUnicode = Default, $hDll = -1)
	...
	If _Console_ReadInput($hConsoleInput, DllStructGetPtr($tInputRecord), $fUnicode, $hDll) = 0 Then Return SetError(@error, @extended, 0)
...
Func _Console_ScrollScreenBufferEx($hConsoleOutput, _
	...
	Local $aResult = DllCall($hDll, "bool", "ScrollConsoleScreenBufferW", _
			"handle", $hConsoleOutput, _
			"ptr", $pScrollRect, _
			"ptr", $pClipRect, _
...
Func _Console_SetTitle($sConsoleTitle, $fUnicode = Default, $hDll = -1)
	...
	Local $aResult = DllCall($hDll, "bool", "SetConsoleTitleW", _
			"wstr", $sConsoleTitle)
	If @error Then Return SetError(@error, @extended, False)
...
Func _Console_WriteConsole($hConsole, $sText, $fUnicode = Default, $hDll = -1)
	...
	Else
		...
		Local $aResult = DllCall($hDll, "bool", "WriteConsoleW", _
				"handle", $hConsole, _
				"wstr", $sText, _
...
Func _Console_WriteOutputCharacter($hConsole, $sText, $iX, $iY, $fUnicode = Default, $hDll = -1)
	...
	Local $aResult = DllCall($hDll, "bool", "WriteConsoleOutputCharacterW", _
			"handle", $hConsole, _
			"wstr", $sText, _
			"dword", StringLen($sText), _
			"dword", BitShift($iY, -16) + $iX, _
